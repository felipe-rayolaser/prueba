...
if ( ! class_exists( 'YITH_WCCL_Admin' ) ) {
	...
	class YITH_WCCL_Admin {
		 ...
		 * @since 1.0.0
		 */
		public $doc_url = 'https://docs.yithemes.com/yith-woocommerce-color-label-variations';
		...
		public function action_links( $links ) {
			$links[] = '<a href="' . admin_url( "admin.php?page={$this->_panel_page}" ) . '">' . __( 'Settings', 'yith-woocommerce-product-add-ons' ) . '</a>';
		...
		public function plugin_row_meta( $plugin_meta, $plugin_file, $plugin_data, $status ) {
			...
			if ( defined( 'YITH_WCCL_INIT') && YITH_WCCL_INIT == $plugin_file ) {
				$plugin_meta[] = '<a href="' . $this->doc_url . '" target="_blank">' . __( 'Plugin Documentation', 'yith-woocommerce-product-add-ons' ) . '</a>';
			}
